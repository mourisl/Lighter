...
A read position overlaps up to $x$ $k$-mers where $1\le x\le k$. If the position was altered by a sequencing error, the overlapping $k$-mers are incorrect and likely weak. These weak $k$-mers are not likely to be sampled by Lighter in the first pass, so we can set a threshold and for this position if the number of $k$-mers that are stored in Bloom filter $A$ is less than this threshold we say this position is untrusted, otherwise we say it is trusted.
