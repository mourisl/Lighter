...
\myworries{Li: when you say we call the position ambiguous, does that mean we don't edit it?}
...
A key parameter of \tool is the \kmer length $k$.  Smaller $k$ yields higher probability that a \kmer affected by a sequencing error also appears elsewhere in the genome.  For larger $k$, the fraction of \kmers that are correct decreases, which could lead to fewer correct \kmer in Bloom filter $A$.  We measured how different settings for $k$ affect accuracy using the \myworries{Li: please say which dataset we used}.  Results are shown in Figure \ref{fig:kmerLength}.  Accuracy is high for \kmer lengths ranging from about 18 to 30.
