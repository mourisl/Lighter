...
Thirdly, we show that the number of positions that contain error and are falsely set as trusted position does not increase. The $x$ kmers from this position are true weak kmers. The null hypothesis stays almost the same again. I will use $\gamma''_1$ to represent the expected multiplicity of true weak kmers in scenario 1 and $\gamma''_2$ for scenario 2. And $\gamma' - \gamma''_1 < z\gamma' - \gamma''_2$, because the increasing speed of multiplicity of true weak kmers is very slow. Therefore, the difference of the expected value between the binomial distribution of the null hypothesis and the binomial distribution for the true weak kmers increases. So the probability of such position falsely be classified as trusted does not increase. Though in scenario 2, the number of such position is roughly $z$ times as large as scenario 1, the difference between the expected values increases so fast, as a result even the number of falsely being classfieid as trusted should not increase. 
