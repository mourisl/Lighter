...
If more than one candidate substitution is equally good, we call position $i+k-1$ ambiguous and just ignore it. if $i+1$ is not close to the end of the read, we will apply the correction procedure on the part from $i+1$ to the end of the read from scratch. Otherwise, Lighter gives up correcting this read. Moreover, the correction on the substring will not call the correction procedure recursively. In other words, if there is another amiguous position between $i+1$ and $|r|$, Lighter will give up correction no matter what.
...
If $k_b$ is not the leftmost \kmer in the read, we apply a similar procedure but moving leftward.
