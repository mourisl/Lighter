...
\begin{enumerate}
\item Department of Computer Science, Johns Hopkins University
\item McKusick-Nathans Institute of Genetic Medicine, Johns Hopkins University
\end{enumerate}
...
With these advances have come an explosion of new software for analyzing large sequencing datasets.
...
For error correction software to be useful in practice, it must make economical use of time and memory even when input datasets are extremely large (billions of reads) and when the genome under study is also large (mammalian).
...
SHREC \cite{schroder2009shrec} and HiTEC \cite{ilie2011hitec} build a suffix index of the input reads and locate errors by finding instances where a substring is followed by a character less often than would be expected if the character were correct.
...
$k$-mers with low multiplicity (\emph{weak} $k$-mers) are systematically edited into solid $k$-mers using a dynamic-programming solution to the spectral alignment problem \cite{pevzner2001eulerian, chaisson2004fragment} or, more often, a fast heuristic approximation thereof.
...
CUDA-EC \cite{shi2010parallel} was novel for its use of Bloom filters as space-efficient alternatives to hash tables for counting $k$-mers and for representing the set of solid $k$-mers.
More recent tools such as Musket \cite{liu2013musket} and BLESS \cite{heo2014bless} also use Bloom filters, sometimes in concert with a hash table, to count $k$-mers or to represent the set of solid $k$-mers.
...
\emph{Lighter} then uses a statistical test applied to each position of each read to compile a set of solid $k$-mers, which are stored in a second Bloom filter.
These two Bloom filters are the only sizable data structures used by \emph{Lighter}.
\emph{Lighter}'s crucial advantage is that its parameters can be set such that the Bloom-filter size and the overall accuracy of the approach are near-constant with respect to the depth of coverage.
In other words, no matter how deep the coverage, \emph{Lighter} can allocate the same sized Bloom filters and achieve nearly the same (a) Bloom filter occupancy, (b) Bloom filter false positive rate, and (c) error correction accuracy.
\emph{Lighter} does this without using any secondary memory (e.g. magnetic or solid-state disk space), in contrast to methods like BLESS, which uses more secondary memory as the size of the input data increases.
\emph{Lighter} is also very simple and fast; faster than all other tools tried in our experiments.
These advantages make \emph{Lighter} quite practical compared to counting-based approaches, all of which require an amount of memory and/or secondary storage that increases with depth of coverage.
...
Recall	& musket	& 92.61	& 92.04	& 92.60	& 92.05	& 92.60	& 92.03 \\ \cline{2-8}
	...
	& lighter	& 99.40	& 98.00	& 99.33	& 98.73	& 99.38	& 98.88 \\ \hhline{|=|=|=|=|=|=|=|=|}
...
F-score	& musket	& 96.06	& 95.68	& 96.05	& 95.69	& 96.05	& 95.68 \\ \cline{2-8}
	...
	& lighter	& 99.26	& 98.56	& 99.21	& 98.94	& 99.22	& 99.02 \\ \hhline{|=|=|=|=|=|=|=|=|}
...
Gain	& musket	& 92.40	& 91.70	& 92.39	& 91.71	& 92.39	& 91.69 \\ \cline{2-8}
	...
	& lighter	& 98.51	& 97.15	& 98.42	& 97.89	& 98.44	& 98.05 \\ \hhline{|=|=|=|=|=|=|=|=|}
